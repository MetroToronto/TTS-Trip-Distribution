// ===== report.js =====
// Simple printable report from Routing.getAllResults().
// Exposes window.Report.openPrintableModal(results).

(function () {
  function openPrintableModal(results) {
    const list = (results || []).filter(r => (r.routes || []).length > 0);
    if (!list.length) {
      Routing.showToast("No routes to print yet.");
      return;
    }
    ...
    const html = `
      <div style="font:14px/1.4 system-ui">
        ...
        <div style="margin-top:12px;font-size:12px;color:#666">
          Note: This summary uses cached ORS routes currently on the map. Street-by-street tables can be added later.
        </div>
      </div>
    `;

    const { close } = (function openModal() {
