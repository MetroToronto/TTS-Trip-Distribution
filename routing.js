/* routing.js — stable build based on your proven version
   Changes from your Oct 14 10AM file:
   1) Minimal normalizeName() (no aggressive rewrites)
   2) Permissive isHighwayName() (Hwy/ON-401/Expressway/Parkway/DVP/QEW/Gardiner)
   3) Highways are NEVER dropped by the min-fragment filter
   Everything else (UI, one-call-per-PD, no Snap/no extra_info, cutoff at first highway)
   stays the same to avoid regressions.
*/
(function (global) {
  // ===== Tunables ===========================================================
  ...
  function avgHeadingBetween(sampled, iStart, iEnd, capM=BOUND_LOCK_WINDOW_M){
    // Vector-average bearings measured from NORTH
  ...
  // Keep minimal — do NOT rewrite highways
  ...
  // Permissive highway detector (accepts Hwy/ON-401/Expressway/Parkway/DVP/QEW/Gardiner/etc.)
  ...
  // If step.name is "-", try to pull something useful from the instruction
  ...
  // ===== Movement builder (cut off at first highway) =======================
  ...
  function buildMovementsFromDirections(coords, steps){
    ...
    for (let si=0; si<steps.length; si++){
      ...
      if (nm){
        pushRow(nm, i0, i1);
        if (isHighwayName(nm)) break; // stop after first highway/expressway
      }
    }
    return rows;
  }
